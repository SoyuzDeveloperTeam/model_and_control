...
SendToBum(0x000002A8, 1, 1);       //Команда в БУМ ()
...
if (KSP_Let[4]){  // Ж
USO_Booled[5][0]=true;
SendToBum(0x0000029A, 1, 1);       //Команда в БУМ ()
iResult = send( TeleSocket,(char *)&send_tru,20, 0  );   //
if (iResult == SOCKET_ERROR) GetWsaError(WSAGetLastError());
...
if (KSP_Let[6]){  // К
KSP_Booled[7][0]=true;
SendToBum(0x000002B0, 1, 1);       //Команда в БУМ ()
iResult = send( TeleSocket,(char *)&send_tru,20, 0  );   //
if (iResult == SOCKET_ERROR) GetWsaError(WSAGetLastError());
...
if (KSP_Let[7]){  // Л
KSP_Booled[8][0]=true;
SendToBum(0x00000281, 1, 1);                     //Команда в БУМ (РО ДК - вкл)
iResult = send( TeleSocket,(char *)&send_tru,20, 0  );   //
if (iResult == SOCKET_ERROR) GetWsaError(WSAGetLastError());
...
SendToBum(0x00000286, 1, 1);       //Команда в БУМ ()
iResult = send( TeleSocket,(char *)&send_tru,20, 0  );   //
if (iResult == SOCKET_ERROR) GetWsaError(WSAGetLastError());
...
SendToBum(0x00000280, 1, 1);                     //Команда в БУМ (ВСК - причал)
iResult = send( TeleSocket,(char *)&send_tru,20, 0  );   //
if (iResult == SOCKET_ERROR) GetWsaError(WSAGetLastError());

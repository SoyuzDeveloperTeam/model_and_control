...
double JDJ2K = DayFromJD(JulDate);
Jou("Результат перевода в Юлианскую Дату - "+FloatToStr(JulDate));
Jou("Прошло с J2000 - "+FloatToStr(JDJ2K));
double Lsun = SredDolVosSun(JDJ2K);
Jou("Средняя долгота Солнца - "+FloatToStr(Lsun));
double SA = Srenanom(JDJ2K);
Jou("Средняя аномаляи - "+FloatToStr(SA));
double Ecl = eclipticdolg(Lsun,SA);
Jou("Эклептичность - "+FloatToStr(Ecl));
double r_sun = rsun(SA);
